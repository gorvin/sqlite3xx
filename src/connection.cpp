...
connection::~connection( ) {
	...
	if( rt != SQLITE_OK ) {
		std::wcerr << "ERROR in destructor of connection: " << rt << std::endl;
	}
}

extern "C" void profiling_callback( void *a, const char *b, sqlite3_uint64 c ) {
	SQLITEXX_UNUSED( a );
	std::cout << "TRACE: " << b << " (time: " << c / 1000 << " ms)" << std::endl;
}
...
result connection::prepared_exec( const string& name ) {
	...
	if( _trace ) std::cout << "TRACE: exec '" << name << "'" << std::endl;
...
prepare::declaration connection::prepare( const string& name, string sql ) {
	...
	if( _trace ) std::cout << "TRACE: prepared '" << name << "' as '" << sql << "'" << std::endl;
...
void connection::prepare_setparam( const string& name, const int pos, const int value ) {
	prepared_stmt* stmt = find_prepared( name );
	if( _trace ) std::cout << "TRACE: prepared '" << name << "' param '" <<
		pos << "'" << " value: " << value << std::endl;
...
void connection::prepare_setparam(const string& name, const int pos, const unsigned char* value, int bycount) {
	prepared_stmt* stmt = find_prepared(name);
	if (_trace) std::cout << "TRACE: prepared '" << name << "' param '" <<
		pos << "'" << " value: " << std::string((char const*)value, bycount).c_str() << std::endl;
	stmt->setparam(pos, value, bycount);
}
