...
prepare::invocation& prepare::invocation::setparam( const std::basic_string<unsigned char>& value, bool nonnull ) {
	SQLITEXX_UNUSED(nonnull);
	_pos++;
	_c.prepare_setparam(_stmt, _pos, &value[0], (int)value.size());
	return *this;
}
...
prepared_stmt::~prepared_stmt( ) {
	assert( _stmt != NULL );
	if( false /* TODO: access to _c.trace( ) */ ) std::cout << "TRACE: finalizing prepared statement '" << _sql << "'" << std::endl;
...
void prepared_stmt::setparam( const int pos, const char* v ) {
	int rc;
	assert( _stmt != NULL );
	rc = sqlite3_bind_text( _stmt, pos, v, strlen( v ), SQLITE_TRANSIENT);
	if( rc != SQLITE_OK ) {
		ostringstream s;
		s << "sqlite3::prepared_stmt::setparam bind error: "
		  << sqlite3_errmsg( _db ) << " (rc: " << rc << ")";
...
void prepared_stmt::setparam(const int pos, const unsigned char* v, int bycount) {
	int rc;
	assert(_stmt != NULL);
	rc = sqlite3_bind_text(_stmt, pos, (char const*)v, bycount, SQLITE_TRANSIENT);
	if (rc != SQLITE_OK) {
		ostringstream s;
		s << "sqlite3::prepared_stmt::setparam bind error: "
			<< sqlite3_errmsg(_db) << " (rc: " << rc << ")";
